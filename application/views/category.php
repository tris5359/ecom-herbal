...
<?php include 'includes/header.php'; ?>
	...
	<title><?php echo $site[0]['sitename'] ?> - Category</title>
<body class="animsition">
  ...
  try{        
	  $jumlah = $this->db->query("SELECT COUNT(*) As jumlah FROM tb_barang where kodedetil ='".$catid."'")->result_array();
					...
					<div class="row isotope-grid">
						<?php             	           
			                try{
	  						  ...
	  						  $stmt = $this->db->query("SELECT * FROM tb_barang where kodedetil ='".$catid."' LIMIT  ".$offset.", ".$tampil."")->result_array();
			                  foreach($stmt as $row){
