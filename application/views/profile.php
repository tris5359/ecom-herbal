...
<?php include 'includes/header.php'; ?>
	<title><?php echo $site[0]['sitename'] ?> - Profile</title>
<body class="animsition" >
	...
	<!-- Content page -->
	<section class="bg0 " style="margin-top: 80px">
		<div class="container" style="box-shadow: 1px 20px 30px 2px #ccc">
			...
			if(empty($this->session->userdata('id'))){
				...
				<div class="col-md-6 p-lr-70 p-t-55 p-b-70 p-lr-15-lg w-full-md">
					<form action="proses/validasi" method="POST" >
		          	...
		          	<div class="col-md-3" >
			            ...
			            <div class="wrap-item-gallery " style="text-align: center;">
			              ...
			              style="margin-left: 100%">
			                <img src="<?php echo base_url()?>assets/images/<?php echo (!empty($user[0]['foto']))? $user[0]['foto'] : 'profile.jpg'?>" width="200" height="200" style="position: absolute;border-radius: 100px;margin-top: -30px;box-shadow: 1px 5px 20px 1px #333;border:2px solid #fff">
			              </a>
			            </div>  
		          	</div>
		          	<div class="col-md-7" style="color: #fff;">
		          		 <div > 
                               <table style="font-family:andalus;font-size:16px;color: #999;width: 100%">
                                   <tr>
                                    <td><p style="margin-top: 20px; font-size:28px;font-family:andalus;color:#fff;position: relative;"><?php echo $user[0]['namadepan'].' '. $user[0]['namabelakang']; ?>
                                     </p></td>
                                    </tr>
                                    <tr>
                                   <td style="width:60px" class="p-t-6"><i class="fa fa-phone" ></i> <p style="margin-top:-27px;margin-left:30px;"><?php echo (!empty($user[0]['nomorhp'])) ? $user[0]['nomorhp'] : 'No Telepon belum di update'; ?></p ></td>
                                   </tr>
                                   <tr>
                                  <td style="width:60px" class="p-t-6"><i class="fa fa-envelope" ></i> <p style="margin-top:-27px;margin-left:30px;"><?php echo (!empty($user[0]['email'])) ? $user[0]['email'] : ' Email belum di update '; ?></p ></td>
                                 </tr>
                                 <tr>
                                 <td style="width:60px" class="p-t-6"><i class="fa fa-map-marker" ></i> <p style="margin-top:-27px;margin-left:30px;"><?php echo (!empty($user[0]['alamat'])) ? $user[0]['alamat'] : 'Alamat belum di update'; ?></p ></td>
                                 </tr>
							</table>   
                         </div>
		          	</div>
			...
			<div class="box box-solid p-lr-50 p-tb-72" style="font-family: andalus">
	        			<div class="box-header with-border m-b-10">
	        				<h4 class="box-title"><i class="fa fa-calendar"></i> <b style="color: #568517">Daftar Transaksi</b></h4>
	        			</div>
	        			<div class="box-body" style="font-size: 16px">
	        				<table class="table table-bordered" id="example1">
	        					...
	        					<?php
	        						...
	        						try{
	  									$stmt = $this->db->query("SELECT * FROM tb_pemesanan WHERE kodepelanggan='".$user[0]['kduser']."' ORDER BY tglbeli DESC")->result_array();
	        							foreach($stmt as $row){
					              ...
					              <?php
	  									$stmt3 = $this->db->query("SELECT * FROM vnota LEFT JOIN tb_barang ON tb_barang.kodebarang=vnota.kodebarang WHERE idpemesanan='".$row['idpemesanan']."'")->result_array();
	        								...
	        								echo '<div class="row m-lr-10">
										        	...
										        	<div class="col-md-2 table-bordered p-t-6 p-b-6">'.number_format($row3['harga'],0,',','.').'</div>
										          	<div class="col-md-2 table-bordered p-t-6 p-b-6">x'.$row3['banyak'].'</div>
										            <div class="col-md-3 table-bordered p-t-6 p-b-6">'.number_format($subtotal,0,',','.').'</div>
										           </div>';	}?>
								              <div class="row m-lr-10 ">
								              	<div class="col-md-9 table-bordered p-t-6 p-b-6">Total</div>
								              	<div class="col-md-3 table-bordered p-t-6 p-b-6">Rp. <?php echo number_format($total,0,',','.')?></div>
								              </div>
								            </div>
